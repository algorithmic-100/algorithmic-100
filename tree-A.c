...
bool insert(Structure * const structure, unsigned int insertValue){
   ...
   // printf("HERE \n");
    ...
    if (structure->count > 0){
        ...
        if (last->val > insertValue){
           // printf("INSERTING INTO LEFT \n");
            last->left = node;
        } else {
           // printf("INSERTING INTO RIGHT \n");
            last->right = node;
        }
        
        indexa += 1;
       // printf("INDEXA %d", indexa);
...
int TREE_GET_OCCURANCES(Structure * const root, /*Node data*/ unsigned int val) {
    ...
    while (next != NULL){
        ...
        printf("INSIDE \n");
        bool isLess = next->val < val;
        bool isMore = next->val > val;
        
        //int cmp = strcmp(&next->name, data.name);
        
        if (/*cmp < 0*/ isLess){
        next = next->right;
        } else if (/*cmp > 0*/ isMore){
            next = next->left;
        } else {
            occurances++;
            dataa = &next;
...
void search(Node * root, unsigned int searching){
    if (root == NULL){
        return;
    }
    
    search(root->left, searching);
    search(root->right, searching);
    
    if (root->val == searching){
        printf("\n \n FOUND %d \n \n", searching);
    }
}
...
int main(int argc, const char * argv[]) {
    ...
    insert(&structure, 1);
    insert(&structure, 5);
    insert(&structure, 2); // 2 or 3 :D
    insert(&structure, 4);
    insert(&structure, 5);
    insert(&structure, 5);
    
    search(structure.root, 5);
    ...
    printElements(structure.root);
    
    
    int occurs = TREE_GET_OCCURANCES(&structure, 5);
    printf(" \n N.O.: %d", occurs);
